...
# 공 크기에 따른 속도
ball_speeds_y = [-0.75, -0.7, -0.65, -0.6]
...
while running:
    ...
    for ball_idx, ball_val in enumerate(balls):
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                weapon_to_remove = weapon_idx   # 삭제할 무기 인덱스
                ball_to_remove = ball_idx       # 삭제할 공 인덱스

                # 가장 작은 크기의 공이 아니라면 다음 단계의 공으로 나눠준다.
                if ball_img_idx < 3:
                    # 현재 공 크기 정보를 가지고 온다.
                    ...
                    balls.append({
                        ...
                        "to_x": -0.1,
                        "to_y": -0.05,
                        "init_speed_y": ball_speeds_y[ball_img_idx + 1]
                    })
                    # 오른쪽으로 튕겨나가는 작은 공
                    balls.append({
                        "pos_x": ball_pos_x + (ball_width / 2) - (small_ball_width / 2),
                ...
                break
        else:   # 무기와 공 충돌 처리 for 문이 끝난 후 이쪽으로 온다.
            continue
        break   # 안쪽 for 문에서 break를 만나면 여기로 진입이 가능하다. (2중 for문을 한번에 탈출한다.)
    ...
    screen.blit(timer, (10, 10))    # 10, 10 위치에 타이머 그려준다.
